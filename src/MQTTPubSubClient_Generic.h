...
#define MQTT_PUBSUB_CLIENT_GENERIC_VERSION            "MQTTPubSubClient_Generic v1.2.0"

#define MQTT_PUBSUB_CLIENT_GENERIC_VERSION_MAJOR      1
#define MQTT_PUBSUB_CLIENT_GENERIC_VERSION_MINOR      2
#define MQTT_PUBSUB_CLIENT_GENERIC_VERSION_PATCH      0

#define MQTT_PUBSUB_CLIENT_GENERIC_VERSION_INT        1002000
...
#include "MQTTPubSubClient_Generic_Debug.h"

extern "C"
{
#include "MQTTPubSubClient_Generic/lwmqtt/lwmqtt.h"
}

#include "MQTTPubSubClient_Generic/util/ArxTypeTraits/ArxTypeTraits.h"
#include "MQTTPubSubClient_Generic/util/ArxContainer/ArxContainer.h"

////////////////////////////////////////

#if ( defined(WEBSOCKETS_H_) || defined(WEBSOCKETS_GENERIC_H_) )
  #if !defined(MQTTPUBSUBCLIENT_USE_WEBSOCKETS)
...
class PubSubClient
{
  ...
  public:
    ...
    bool connect(const String& client_id, const String& user = "", const String& pass = "", const uint32_t timeout_override_ms = 0)
    {
      ...
      MQTT_LOGDEBUG1("connect: last_error =", last_error);
    ...
    bool publish(const String& topic, const String& payload, const bool retained = false, const int& qos = 0)
    {
    ...
    bool publish(const String& topic, uint8_t* payload, const size_t length, const bool retained = false,
                 const uint8_t qos = 0)
    {
      if (!isConnected())
      {
        MQTT_LOGERROR("PubSubClient::publish: Error, not connected");
      ...
      if (last_error != LWMQTT_SUCCESS)
      {
        MQTT_LOGERROR("PubSubClient::publish: Error, not LWMQTT_SUCCESS");
    ...
    bool subscribe(const String& topic, const uint8_t& qos, const topic_callback_t& cb)
    {
      ...
      if (last_error != LWMQTT_SUCCESS)
      {
        MQTT_LOGERROR("PubSubClient::subscribe: Error, not LWMQTT_SUCCESS");
    ...
    bool unsubscribe(const String& topic)
    {
      if (!isConnected())
      {
        MQTT_LOGERROR("PubSubClient::unsubscribe: Error, not connected");
      ...
      if (last_error != LWMQTT_SUCCESS)
      {
        MQTT_LOGERROR("PubSubClient::unsubscribe: Error, not LWMQTT_SUCCESS");
    ...
    bool update()
    {
      if (!isConnected())
      {
        MQTT_LOGERROR("PubSubClient::update: Error, not connected");
      ...
      if (available > 0)
      {
        ...
        if (last_error != LWMQTT_SUCCESS)
        {
          MQTT_LOGERROR("PubSubClient::update: Error1, not LWMQTT_SUCCESS");
      ...
      if (now_ms > prev_keep_alive_ms + keep_alive_interval_ms)
      {
        ...
        if (last_error != LWMQTT_SUCCESS)
        {
          MQTT_LOGERROR("PubSubClient::update: Error2, not LWMQTT_SUCCESS");
    ...
    bool disconnect()
    {
      if (!isConnected())
      {
        MQTT_LOGERROR("PubSubClient::disconnect: Error, not connected");
  ...
  private:
    ...
    static lwmqtt_err_t lwmqtt_arduino_network_read(void* ref, uint8_t* buffer, size_t len, size_t* read,
                                                    uint32_t timeout_ms)
    {
    ...
    static lwmqtt_err_t lwmqtt_arduino_network_write(void* ref, uint8_t* buffer, size_t len, size_t* sent,
                                                     uint32_t /*timeout_ms*/)
    {
      // cast network reference
      auto n = (lwmqtt_arduino_network_t*)ref;
    ...
    static void lwmqtt_arduino_mqtt_client_handler(lwmqtt_client_t* /*client*/, void* ref,
...
#endif    // #ifndef MQTT_PUBSUB_CLIENT_GENERIC_H
